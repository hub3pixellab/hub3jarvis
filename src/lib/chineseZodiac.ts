 ...
 * O animal muda no Ano Novo Chinês (entre 21/01 e 21/02), não em 1º de janeiro.
 */

/** Data do Ano Novo Chinês por ano gregoriano, como [mês, dia] (fonte: Helmer Aslaksen / romanization.com, 1900–2099). */
...
/**
 * Deriva o signo do zodíaco chinês a partir de uma data ("YYYY-MM-DD" ou ISO).
 * Antes do Ano Novo Chinês do ano de nascimento, o animal é o do ano anterior.
 */
export function getChineseZodiacSign(value: string | Date | null): ChineseZodiacSignKey | null {
  ...
  const effectiveYear =
    cny && (month < cny[0] || (month === cny[0] && day < cny[1])) ? year - 1 : year;

  // Ciclo de 12 anos: 1900 = Rato (índice 0), 1901 = Boi (1), ...
  const index = ((effectiveYear - 1900) % 12 + 12) % 12;
  return (
    [
      "rato",
      "boi",
