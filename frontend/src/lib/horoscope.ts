 ...
 * O texto é composto por três partes traduzíveis:
 *   essência do signo + clima do dia + conselho do dia
...
export function getDailyHoroscope(
  ...
  // Offset por signo para que signos diferentes não recebam o mesmo texto.
  const toneIndex = (day + signIndex * 3) % TONE_COUNT;
  ...
  return {
    sign,
    toneKey: `horoscope.tone.${toneIndex}`,
